...
def condense_materials(clean_df, sort = True):
  try:
    ...
    condensed_df = clean_df.groupby("Material", as_index=False)[["Invoiced Quantity", "NET SALES", "Cost"]].sum()
...
def directory_to_precluster(sort_rev = True):
   # select the raw data
...
print(directory_to_precluster(sort_rev=True))
